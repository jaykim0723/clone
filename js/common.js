...
function initNavigation() {
	...
	for (var i=0;i<lists.length;i++) {
		...
		lists[i].onmouseover = navMouseOver; //gnb 에 마우스 오버시 navMouseOver 실행
		//lists[i].onmouseover = $("#gnb").removeClass( "gnb_all" ).addClass( "gnb" );
		lists[i].onmouseout = navMouseOut; //gnb 에 마우스 아웃시 navMouseOut 실행
...
function navMouseOver(e) {
	...
	if (evtTarget.relatedItem && !evtTarget.relatedItem.opened) {
		...
		currentlyOpenedMenus.push(evtTarget.relatedItem); // currentlyOpenedMenus 배열에서 evtTarget 와 relatedItem 를 추가
